		...
		if self.filters.get("type_of_business") ==  "B2C Small":
			...
			for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
				...
				for rate, items in items_based_on_rate.items():
					...
					tax = frappe.db.sql(""" select account_head,tax_amount from
						 `tabSales Taxes and Charges` where parent="%s" """%row[2],as_dict=1)
					t1 =0.0
					t2 =0.0

					""" Taxses Extra Coloumn Added Logic By -----Harshal"""

					del row[8:10]

					for d in tax:
						if d['account_head'] =="CGST Payable - ITL":
							t1 += d['tax_amount']
							row.insert(9,t1)
							row.insert(10,t1)
							row.insert(11,0)
							row.pop(12)
						elif d['account_head'] =="IGST Payable - ITL":
							t2 += d['tax_amount']
							row.insert(9,0)
							row.insert(10,0)
							row.insert(11,t2)
	...
	def get_b2cs_data(self):
		...
		for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
			...
			for rate, items in items_based_on_rate.items():
				...
				net_bal = invoice_details.get("base_grand_total")
				invoice = invoice_details.get("invoice_number")

				tax = frappe.db.sql(""" select account_head,tax_amount from
                                                 `tabSales Taxes and Charges` where parent="%s" """%invoice,as_dict=1)
				t1 =0.0
				t2 =0.0
				for d in tax:
					if d['account_head'] == "CGST Payable - ITL":
						...
						t2 += d['tax_amount']
					#print("lllllllllllllllllllllllllllllllll",tax)
					...
					#if tax[0] == "CGST Payable - ITL":
					row['net_value']  +=net_bal
					row['cgst'] += t1
					row['sgst'] += t1
					#else:
					row['igst'] += t2
	...
	def get_row_data_for_invoice(self, invoice, invoice_details, tax_rate, items):
		...
		for item_code, net_amount in self.invoice_items.get(invoice).items():
				if item_code in items:
					...
					#	print("llllllllllllllll",taxable_value,tax_rate)
					elif not self.item_tax_rate.get(invoice):
						taxable_value += abs(net_amount)
#						print("2222222222222",taxable_value,tax_rate)
	...
	def get_items_based_on_tax_rate(self):
		...
		#print("ppppppppppppppppppppp",self.tax_details)
