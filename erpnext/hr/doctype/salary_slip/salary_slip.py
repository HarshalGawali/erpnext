...
class SalarySlip(TransactionBase):
	...
	def calculate_net_pay(self):
		...
		self.gross_pay = self.get_pf("earnings")
		self.total_deduction = self.get_pf("deductions")
		self.total_deduction = (self.total_deduction)
	...
	def eval_tax_slab_condition(self, condition, data):
		try:
			...
			frappe.throw(_("Name error: {-1}".format(err)))
		except SyntaxError as err:
	...
	def get_component_totals(self, component_type):
		...
		for d in self.get(component_type):
			if not d.do_not_include_in_total:
				...
				total += d.amount
		return total
	def get_pf(self, component_type):
		total = 0.0
		hra = 0.0
		shft = 0.0
		pf_1 =0.0
		...
		amount_add= 0.0
				...
				if additional_component.type == "Earning":
                                	amount_add += additional_component.amount
				else:
					amount_add +=0
                                overwrite = additional_component.overwrite

		#amount_add -= 12
		#additional_comp = add_additional_salary_components()
		for d in self.get(component_type):
			if not d.do_not_include_in_total:
				d.amount = flt(d.amount, d.precision("amount"))
				total += d.amount
				if d.abbr == "HRA":
					amt =0.0
					d.amt=flt(d.amount, d.precision("amount"))
					d.amount = d.amt
					hra = d.amt
					self.abc = hra
				if d.abbr == "OT":
				...
				if d.abbr == "PF":
					d.amt = flt(d.amount,d.precision("amount"))
					if self.gross_pay - self.abc - self.inn -self.sb-  amount_add < 15000:
						d.amount = round(flt((self.gross_pay -self.sb - self.abc - self.inn - amount_add)*.12))
						pf_1 += d.amount - 1
						shft = d.amount
						total += pf_1
					elif self.gross_pay - self.abc - amount_add >= 15000:
						d.amount = 1800
						pf_1 += d.amount -1
						total += pf_1
				...
				if d.abbr == "PT":
                                        ...
                                        if self.gross_pay > 10000:
                                                d.amount = 200
                                                total += d.amount -1
                                        elif self.gross_pay < 10000:
						if self.gender =="Male":
							if self.gross_pay  < 7500:
                                                		d.amount = 0
							else:
								d.amount = 175
                                                	total += d.amount -1
			...
			if d.do_not_include_in_total:
				if d.abbr == "ECPF":
					d.amt = flt(d.amount,d.precision("amount"))
					if self.gross_pay - self.abc -self.sb -self.inn - amount_add <= 15000:
						d.amount = round(flt((self.gross_pay -self.sb- self.abc - self.inn - amount_add)*.12))

					elif self.gross_pay - self.abc - self.inn -amount_add> 15000:
						d.amount = 1800
				if d.abbr =="ESIC EC":
					d.amt = flt(d.amount,d.precision("amount"))
					test1 = math.ceil(d.amt)
                                        d.amount = test1
                                        total -= d.amt
                                        total +=d.amount
...
#				if d.abbr != "PF":
#                                        d.amt = flt(d.amount,d.precision("amount"))
#                                        if self.gross_pay - self.abc < 15000:
#                                                d.amount = flt((self.gross_pay - self.abc)*.12)
#                                                total +=  d.amount
#                                        elif self.gross_pay - self.abc > 15000:
#                                                d.amount = 1800
 #                                           
 #                                               total += d.amount
		self.gross_pay = round(self.gross_pay)
		return total
