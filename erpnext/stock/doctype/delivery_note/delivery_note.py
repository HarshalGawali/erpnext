...
@frappe.whitelist()
def ip_passing(item_name=None,order=None,ip=None):
        ips = ip
        order = order

        frappe.db.sql("""update `tabSales Order Item` set ip='%s' where  parent ='%s' and item_name ='%s'"""%(ips,order,item_name))


@frappe.whitelist()
def service_date_passing(item_name = None ,service_end=None,cron_end_date = None,next_inv_dt=None,order=None,service_start_date=None):
        order = order
        service_date = service_start_date
        frappe.db.sql("""update `tabSales Order Item` set  service_start_date='%s' where  parent ='%s' and item_name = '%s'"""%(service_date,order,item_name))
        frappe.db.sql("""update `tabSales Order Item` set  service_end='%s' where  parent ='%s' and item_name = '%s'"""%(service_end,order,item_name))
        frappe.db.sql("""update `tabSales Order Item` set  next_inv_dt='%s' where  parent ='%s' and item_name = '%s'"""%(next_inv_dt,order,item_name))
        frappe.db.sql("""update `tabSales Order Item` set  cron_end_date='%s' where  parent ='%s' and item_name = '%s'"""%(cron_end_date,order,item_name))
	frappe.db.sql("""update `tabSales Order Item` set  service_start_period='%s' where  parent ='%s' and item_name = '%s'"""%(service_date,order,item_name))
	frappe.db.sql("""update `tabSales Order Item` set  service_end_period='%s' where  parent ='%s' and item_name = '%s'"""%(service_end,order,item_name))
@frappe.whitelist()
def domain_passing(item_name = None ,order=None,domain=None):
        order = order
        domain_data = domain
        frappe.db.sql("""update `tabSales Order Item` set  domain='%s' where  parent ='%s' and item_name = '%s'"""%(domain,order,item_name))
...
def make_sales_invoice(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		target.sales_order_no = source.so_no
