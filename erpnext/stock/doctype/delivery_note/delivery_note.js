...
frappe.ui.form.on("Delivery Note Item", {
	...
	cost_center: function(frm, dt, dn) {
	 ...
	 ip: function(frm, dt, dn){

                var d =locals[dt][dn];
                        var value = d.ip
                        var service_date = d.service_start_date
                        console.log(service_date)
        frappe.call({
            method:"erpnext.stock.doctype.delivery_note.delivery_note.ip_passing",
        //     doctype: "Delivery Note Item",
            args: {"item_name":d.item_name,"ip":d.ip,"order":d.against_sales_order}
        //     // route: cur_frm.doc.route,
        //     callback :function(r){

        //         // cur_frm.set_value("route",r.message);
        //     }
        });
        },
        domain: function(frm, dt, dn){
                ...
                var d =locals[dt][dn];
                        ...
                        console.log(service_date)
        frappe.call({
            method:"erpnext.stock.doctype.delivery_note.delivery_note.domain_passing",
        //     doctype: "Delivery Note Item",
            args: {"item_name":d.item_name,"domain":d.domain,"order":d.against_sales_order}
        //     // route: cur_frm.doc.route,
        //     callback :function(r){
	...
	date_passing: function(frm, dt, dn){
                ...
                var d =locals[dt][dn];
                        ...
                        console.log(service_date)
        frappe.call({
            method:"erpnext.stock.doctype.delivery_note.delivery_note.service_date_passing",
        //     doctype: "Delivery Note Item",
            args: {"item_name":d.item_name,"next_inv_dt":d.next_inv_dt,"service_end":d.service_end,"cron_end_date":d.cron_end_date,"domain":d.domain,"service_start_date":d.service_start_date,"order":d.against_sales_order }
        //     // route: cur_frm.doc.route,
        //     callback :function(r){
...
frappe.ui.form.on("Delivery Note Item", "service_start_date", function(frm, cdt, cdn) {
var d = locals[cdt][cdn];

var date = d.service_start_date

var today = new Date(date);
var date3 = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
...
var end = end_date.getFullYear()+'-'+(end_date.getMonth()+1)+'-'+end_date.getDate();
 ...
 var day = d.advance_invoice_day
    date1 =date1.setDate(date1.getDate() - day);

  var end_date1 = today.setDate(today.getDate() + 100);
    end_date1 = new Date(end_date1)
var end1 = end_date1.getFullYear()+'-'+(end_date1.getMonth()+1)+'-'+end_date1.getDate();

frappe.model.set_value(cdt, cdn, "cron_end_date",frappe.datetime.add_months(date, 10000));
if(d.billing_cycle == "One Time")
{

}
if(d.billing_cycle == "Monthly")
{
var date3= d.service_start_date
var now = new Date(date3);
var day = d.advance_invoice_day
now.setDate(now.getDate()- day)

frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 1));
}
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 3));
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 6));
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 12));
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 24));
}
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 36));
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 60));
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 84));
...
frappe.model.set_value(cdt, cdn, "next_inv_dt",frappe.datetime.add_months(now,0));
frappe.model.set_value(cdt, cdn, "service_start_date",frappe.datetime.add_months(date, 0));
frappe.model.set_value(cdt, cdn, "service_end",frappe.datetime.add_months(end, 120));
