...
frappe.ui.form.on("Sales Order", {
	...
	advance_invoice_days: function(frm) {
                $.each(frm.doc.items || [], function(i, d) {
                        if(!d.advance_invoice_day) d.advance_invoice_day = frm.doc.advance_invoice_days;
                });
                refresh_field("items");
        }
...
frappe.ui.form.on("Sales Order Item", {
        item_code: function(frm,cdt,cdn) {
                var row = locals[cdt][cdn];
                if (frm.doc.advance_invoice_days) {
                        row.advance_invoice_day = frm.doc.advance_invoice_days;
                        refresh_field("advance_invoice_day", cdn, "items");
                } else {
                        frm.script_manager.copy_from_first_row("items", row, ["advance_invoice_day"]);
                }
        },
        advance_invoice_day: function(frm, cdt, cdn) {
                if(!frm.doc.advance_invoice_days) {
                        erpnext.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "advance_invoice_days");
                }
        } 

});
frappe.ui.form.on("Sales Order Item", "service_start_date", function(frm, cdt, cdn) {
...
frappe.ui.form.on("Sales Order Item", "status", function(frm, cdt, cdn) {
var d = locals[cdt][cdn];
if(d.billing_cycle == "One Time")
    {
        frappe.model.set_value(cdt, cdn, "next_inv_dt",'')
    }
if(d.billing_cycle == "Monthly")
    {
